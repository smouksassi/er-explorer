...
5. **Slice F — ADR-0013 family adapters** (STARTED, `ba0a065`): ✅ F1 — `EndpointFamilyAdapter` interface in domain (fittedAt/observedSummary/readoutDecimals), `logisticFamily`+`linearFamily` in `apps/demo/src/endpointFamilies.ts`, `updateReadout` converted (family branch gone). **Next:** F2 — convert observedSummary consumers (`computeBinaryStatsFromRows`/linear twin, observed bins, projection callouts); F3 — unify `ProjectedGroup`/`LinearProjectedGroup` renderer input over the adapter (one projection overlay shape); F4 — conformance matrix (same facet×color×split×click scenario per family, structural assertions in smoke + data tests).
