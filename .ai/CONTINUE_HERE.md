...
4. **Slice D — dist geometry (NEXT):** strip once per exposure column + exact-match dedup (kills duplicated stacked strips + overlapping axes); 2-D facet grids.
5. **Slice E — consolidation:** guided presets P1–P3 (delete overlay mount + compare booleans), compare-painter convergence into the single paint path (incl. linear split-row projections in compare cells), runtime Set→ViewSelection rename, split-bin callout declutter (half-bin callouts like "8/9" crowd projection markers — label distinctly or offset), smoke matrix (facet-panels-differ, facet order, one-curve-per-degenerate-panel, strip-N vs projection-N annotation under missingness).
6. **Slice F — ADR-0013 family adapters:** extract `EndpointFamilyAdapter` (fit / curve / fittedAt / observedSummary / readoutFormat); rewrite curve-group + projection + readout pipelines to consume adapters with NO family branching; build the conformance matrix (same facet×color×split×click scenarios per family, structural assertions). This is the prerequisite for Emax / ordinal / bounded-smoother families — do NOT add a new family before F lands.
