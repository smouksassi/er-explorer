...
5. **Slice E — guided presets P1–P3** (delete overlay mount + compare booleans), compare-painter convergence (incl. linear split-row projections in compare cells), runtime Set→ViewSelection rename, smoke matrix (facet-panels-differ, facet order, one-curve-per-degenerate-panel), **and split-bin callout declutter** (user request: when "Group exposures by median/tertiles" + dose-click observed are both on, half-bin callouts like "8/9" crowd the projection markers and read as wrong Ns — label them distinctly, e.g. "n=9 · low half", or offset them).
